...
export default function Navbar(props) {
  ...
  return (
    <nav className={`p-3 flex items-center justify-between z-10`}>
        ...
        <div className={`nav-links md:static absolute md:min-h-fit min-h-[40vh] right-0 top-[-100%] md:w-auto w-full flex items-center`}>
          <div className={`lis font-semibold  flex md:flex-row flex-col md:items-center md:gap-[4vw] gap-6 ml-8 md:ml-0`}>
            ...
            <Link to="/notes" className={`heads ${mode.mode === 'dark' ? 'text-white' : 'text-black'} hover:text-gray-300 px-4 flex`}><span><AiFillBook className='mt-1 mr-1'/></span>Notes</Link>
            <Link to="/lost-and-found/allItems" className={`heads ${mode.mode === 'dark' ? 'text-white' : 'text-black'} hover:text-gray-300 px-4 flex`}><span><AiOutlineAreaChart className='mt-1 mr-1'/></span>Lost|Found</Link>
            <Link to="/contact" className={`heads ${mode.mode === 'dark' ? 'text-white' : 'text-black'} hover:text-gray-300 px-4 flex`}><span><BiMessageSquareDetail className='mt-1 mr-1'/></span>Contact</Link>
          </div>
        </div>
